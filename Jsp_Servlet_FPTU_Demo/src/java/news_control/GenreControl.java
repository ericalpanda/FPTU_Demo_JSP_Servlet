...
public class GenreControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String genreID = request.getParameter("id_genre");
        DAO dao = new DAO();
        List<News> list = dao.getAllNewsByGenre_Id(genreID);
        List<Genre> listG = dao.getAllGenre();
        
        request.setAttribute("listN", list);
        request.setAttribute("listG", listG);
        request.getRequestDispatcher("main_news.jsp").forward(request, response);
        }
